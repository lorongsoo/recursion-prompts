...
var isEven = function(n) {
	...
	return n < 0 ? isEven(n * -1 -2) : isEven(n-2);
};
...
var multiply = function(x, y) {
	...
	if(x < 0 && y < 0){
		return Math.abs(y) + multiply(Math.abs(x) - 1, Math.abs(y)) 
	} else if(x > 0 && y < 0){
		return -1 * Math.abs(y) + multiply(Math.abs(x) - 1, Math.abs(y)) 
	} else if(x < 0 && y > 0){
		return -1 * Math.abs(y) + multiply(Math.abs(x) - 1, Math.abs(y)) 
	} else if(x > 0 && y > 0){
		return Math.abs(y) + multiply(Math.abs(x) - 1, Math.abs(y)) 
...
//IN ORDER TO USE CONCAT, ENSURE RESUlT IS AN ARRAY
...
var countValuesInObj = function(obj, value) {
	...
	for(let key in obj) {
		if(typeof obj[key] !== 'object') {
			...
			sum = sum + countValuesInObj(obj[key], value)
		}
	}
	return sum
};
...
var fibonacci = function(n) {
	...
	if(n === 1) {
	 ...
	 let returnVal = fibonacci(n-1)
	return returnVal.push(returnVal[returnVal.length-1] + returnVal[returnVal.length-2])
};
...
// 	return fibonacci(n-1).push(fibonacci(n-1)[fibonacci(n-1).length-1] + fibonacci(n-1)[fibonacci(n-1).length-2])
// };
...
var augmentElements = function(array, aug) {
	...
	array[0].push(aug)
	//ensure that concating with an array not the element of an array
...
// 	//incorrect. array[0].push(aug) returns the length of the new array
...
var alternateSign = function(array) {
	...
	if(array.length%2===0) {
		return alternateSign(array.slice(0, len-1)).concat(Math.abs(array[len-1]) * -1)
	} else {
		return alternateSign(array.slice(0, len-1)).concat(Math.abs(array[len-1]))
	}
};
...
//parseInt of a character returns NaN
//parseInt of '5' returns 5
//use isNaN to check if something is NaN
	if(isNaN(parseInt(str[0]))) {
		return str[0] + numToText(str.slice(1));
	} else {
		return text[str[0]] + numToText(str.slice(1));
	}
}
